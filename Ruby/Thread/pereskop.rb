#-*- coding: utf-8 -*-  # Very important for RUSSIA and more...
=begin
****************************************************************
...
speed_crt = 4000.0   #  KB/s  criterion min speed for warning 
$err_print = 0    # print error ( 0-Noprint 1-Print->errors )
site_cnt = 0
spd = ""
debug = 1 # for debug print ( 0-no 1- print )
threads = [] # array for threads

yandex = "" # alias string for output
google = ""
dns    = ""
server = ""
	...
	while cnt > i  #  Cycle while 
		...
		then               # !!! ping error !!! 
			...
			if ping_str.empty?      # ping no ->it is not resolved on server in firewall !!!
				err << "!NR!" # Formating string for warning not resolved
				# err_print = 1      # print error if need print error
				return  "#{out}#{err}"  # return string (!NR!)
			end
		
			start_poz=ping_str.index( /\d/ ).to_i
		
			##############          
			kon = start_poz+3
			ping_flt=ping_str[start_poz..kon].to_f  # string to float format
...
while site_cnt < 3  do #  Cycle while
	
	#cmd = "wget -t 2 -T 10 -delete-after -o /opt/logfile.txt http://www.yandex.ru" #2>&1 # old
  ...
  #answer = str[( /\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d \((\d{1,2}\,\d{1,2}\s\w\w\/s|\d{1,3}\s\w\w\/s)/)]
...
#============== hdd =======================     
    ...
    tm_dur = sprintf( "%-6.2f", tm_stop - tm_start ) # Program work time_str
    
##############################################     
    
    tm_stop = Time.now.to_f   # for Program work time
    tm_dur  = sprintf( "%-7.3f", tm_stop - tm_start ) # Program work time
    
# End MAIN  Thread
threads.each {|thr| thr.join } # Wait termination of work of all threads COOL !!! 
    
	# For DEBUG
	if debug == 1
	    out_put = "[#{time_str}]  #{yandex} #{google} #{dns} #{server} spd=#{spd} cpu=#{cpu} md=#{mb}  time=#{tm_dur}"
	    puts out_put
	end
